...
@TestPropertySource(properties = "coupon.strategy=redis-streams")
class RedisStreamsStrategyTest extends IssuanceStrategyTestBase {
    ...
    @Test
    @DisplayName("Redis Streams Consumer가 비동기로 DB에 Issuance를 저장한다")
    void consumerSavesIssuanceToDb() {
        var coupon = couponRepository.save(
            new com.couponrush.domain.coupon.entity.Coupon("STREAMS-DB-TEST", 10));
