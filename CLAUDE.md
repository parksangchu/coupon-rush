...
선착순 쿠폰 발급 시스템. 4가지 동시성 전략을 단계별로 구현하고 k6 부하 테스트로 비교한다.
...
`IssuanceStrategy` 인터페이스를 4개 구현체가 구현한다.
`coupon.strategy` 프로퍼티로 런타임 전환. 코드 변경 없이 동일 조건에서 전략 비교 가능.
전략의 동기와 가설은 `설계문서.md`, 기술 문서는 `docs/` 참고.
...
### 완성보다 이해가 우선이다
이 프로젝트의 목적은 4개 전략을 빠르게 완성하는 것이 아니라,
...
### 설계문서 방향 변경 시 함께 갱신한다
설계문서의 방향이 변경될 때는 기술 문서(docs/)에 변경 사유를 기록하고 설계문서를 함께 갱신한다.
