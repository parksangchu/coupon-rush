...
| Redis Counter | Lua (INCR 원자 연산) | **16ms** | 6,270ms (붕괴) | 1,000 RPS 최고 성능, 2,000에서 한계 |

Redis Counter는 **락 없이** INCR 원자 연산으로 동시성 해결. 1,000 RPS에서 Single UPDATE(54ms)보다 빠르고 VU 10개로 소화. 거절(전체 요청의 90% 이상)이 Redis에서만 처리되어 DB 부하가 크게 줄어든다. 하지만 발급 성공 시 **DB INSERT가 동기로 실행**되어 2,000 RPS에서 병목 (HikariCP Pending 200, CPU 100%).
