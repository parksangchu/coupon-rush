...
Redis Counter는 **락 없이** INCR 원자 연산으로 동시성 해결. 거절(전체 요청의 90% 이상)이 Redis에서만 처리되어 DB 부하가 크게 줄어든다. 2,000 RPS에서 Single UPDATE 대비 p95 -28%, dropped -15%p 개선. 하지만 발급 성공 시 **DB INSERT가 동기로 실행**되어 여전히 병목 (HikariCP Pending 200, CPU 100%).
