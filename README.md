...
| 전략 | 방식 | 1,000 RPS p(95) | 2,000 RPS p(95) |
|------|------|-----------------|-----------------|
| Pessimistic Lock | SELECT FOR UPDATE | 5,468ms (붕괴) | - |
| Single UPDATE | 단일 UPDATE 원자 연산 | **110ms** | 1,464ms |

Single UPDATE가 DB 안에서 할 수 있는 최선. 하지만 **DB row-level lock 직렬화** 때문에 2,000 RPS에서 급격히 느려진다.
...
| 전략 | 방식 | 1,000 RPS p(95) | 2,000 RPS p(95) |
|------|------|-----------------|-----------------|
| Redis Lock | Redisson 분산 락 | 13,053ms (붕괴) | - |
| Redis Counter | Lua (INCR 원자 연산) | **25ms** | 432ms |

Redis Counter는 **락 없이** INCR 원자 연산으로 동시성 해결. Single UPDATE(110ms)보다 빠르고, 거절(전체 요청의 90% 이상)이 Redis에서만 처리되어 DB 부하가 크게 줄어든다. 하지만 발급 성공 시 **DB INSERT가 동기로 실행**되어 5,000 RPS에서 한계.
...
| 전략 | 방식 | 2,000 RPS p(95) | 5,000 RPS p(95) | 처리량 천장 |
|------|------|-----------------|-----------------|-----------|
| Kafka | Lua + Kafka produce | **9.7ms** | **18ms** | 약 7,000 req/s |
| Redis Streams | Lua (INCR + XADD 원자) | **2.6ms** | **9.4ms** | 약 7,000 req/s |

두 전략 모두 5,000 RPS를 p95 20ms 미만으로 소화한다 (Step 2는 2,000에서 이미 432ms).

**처리량 천장은 두 전략 모두 약 7,000 req/s로 동일** (앱 서버 CPU 2 vCPU 한계). 차이는 성능이 아닌 구조적 특성:
