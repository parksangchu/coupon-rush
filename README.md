...
| Redis Counter | Lua (INCR 원자 연산) | **9.2ms** | 1,074ms |
...
|------|------|-----------------|-----------------|-----------|
| Kafka | Lua + Kafka produce | **8.1ms** | **15ms** | 약 7,000 req/s |
| Redis Streams | Lua (INCR + XADD 원자) | **2.3ms** | **12ms** | 약 7,000 req/s |

두 전략 모두 10,000 RPS에서도 p95 20ms 이하 (Step 2는 2,000에서 이미 1,074ms).
...
**해결**: `TransactionTemplate`으로 프로그래매틱 트랜잭션 관리. `execute()` 반환 시점에 커밋이 완료되므로, 그 후 `lock.unlock()`을 호출하면 커밋 → 락 해제 순서가 보장된다.
