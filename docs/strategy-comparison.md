...
### Step 2 시점: 1,000 RPS (쿠폰 10,000개, 30초)

동기 전략 4개의 비교. DB 중복 체크 조건.
...
| Redis Counter | **16ms** | **10** | **0** | 최고 성능 |

### Step 3 시점: 비동기 전략 비교 (쿠폰 10,000개, 30초)

비동기 전략과의 공정 비교를 위해 Redis Counter도 Redis SET 중복 체크로 변경. 이 조건에서 Redis Counter의 성능은 위 표보다 낮아진다 (1,000 RPS p95: 16ms → 3,655ms). DB 중복 체크가 없어진 대신 Redis SET 연산이 추가되고, 동일 조건에서 DB INSERT 병목이 더 부각되기 때문.
...
> **비동기 전략의 1,000 RPS 역전 현상**: Kafka(1,180ms)와 Redis Streams(432ms)의 1,000 RPS가 2,000 RPS(9.9ms, 1.7ms)보다 오히려 느리다. 1,000 RPS에서는 쿠폰 소진에 10초가 걸려 거절 폭주 구간이 길어지고 VU가 누적되면서 latency가 상승한다. 2,000/3,000 RPS에서는 쿠폰이 빨리 소진되어 대부분 Redis에서 즉시 거절되므로 오히려 안정적이다.
