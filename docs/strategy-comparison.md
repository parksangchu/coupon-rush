...
선착순 쿠폰 발급 시스템의 동시성 전략을 4단계에 걸쳐 구현하고, k6 부하 테스트로 비교했다. 각 단계는 이전 단계의 실측 병목을 근거로 진행했다.
...
- **둘 다 필요**: Redis Streams로 시작, 트래픽 증가 시 Kafka로 전환. 전략 패턴 구조라 코드 변경 없이 설정만으로 전환 가능.
