...
### 2026-03-23: 대안 검토 — Step 3 안에서 할 수 있는 개선

2,000 RPS에서 DB INSERT 병목이 발견됐다. 다음 전략(Step 4)으로 넘어가기 전에 현재 전략 안에서 해결할 수 있는지 검토한다.
...
- **JDBC batch INSERT**: 버퍼링 + 주기적 flush가 필요해 사실상 비동기 저장 (Step 4)과 경계가 모호

**결론**: Step 3 안에서 pool size 증가와 Lua 스크립트를 적용하여 2,000 RPS 처리량을 19,135 → 26,026 (+36%)까지 개선했다. 그러나 DB INSERT가 동기인 한 CPU 포화 + HikariCP 경합은 불가피하며, 이것이 Step 4(Kafka 비동기 저장)의 동기가 된다.
